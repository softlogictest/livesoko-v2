...
export const OrderDrawer: React.FC<{ order: OrderCardProps, onClose: () => void }> = ({ order, onClose }) => {
  ...
  const handleSendToRider = () => {
    const message = `*DukaLive Delivery Request*%0A%0A*Customer:* ${order.buyer_name}%0A*Phone:* ${order.buyer_phone}%0A*Item:* ${order.quantity}x ${order.item_name}%0A*Location:* ${order.delivery_location}%0A%0A_Please confirm dispatch._`;
