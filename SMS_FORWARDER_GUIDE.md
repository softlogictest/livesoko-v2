...
This guide will show you how to set up the Android SMS Forwarder application so that every time you receive an M-Pesa payment, it is automatically forwarded to DukaLive for real-time order matching.
...
We need to send the message to the DukaLive Backend.
- **Type**: Select **Webhook** or **Web**.
- **URL**: Paste your unique Webhook URL here from your DukaLive Settings page.
  *(e.g., `https://dukalive-api.railway.app/api/sms/tok_abc123`)*
- **Method**: Select **POST**.
...
### 4. Save and Test
- Hit **Save**.
- Tap **Test Rule** or send yourself a dummy message from "M-PESA" to verify your DukaLive dashboard flashes green.

## Crucial: Battery Optimization Settings
Android aggressively closes apps running in the background. If you do not change these settings, your phone will stop forwarding SMS messages when DukaLive is closed.
