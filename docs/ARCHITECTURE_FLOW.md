# VibeSoko: Architecture & Data Flow 🔄

VibeSoko is designed for **low-latency data movement**. This document outlines how a customer purchase on TikTok Live becomes a verified order on your dashboard.
...
3. **Apps Script**: On form submission, a Google Apps Script triggers. It formats the data and sends a **POST Request** to the VibeSoko API with your unique `webhook_token`.

## 2. The Verification Loop (Backend -> Dash)
1. **API Arrival**: The backend receives the order. It validates the token and the inputs.
2. **Database Write**: The order is saved to `vibesoko.db` with a `PENDING` status.
3. **Real-time Broadcast**: The `SSE Manager` sends a message to all connected clients for that specific `shop_id`.
4. **Instant Notification**: Your VibeSoko PWA vibrates and shows a red "New Order" indicator on the dashboard.

## 3. The Fulfillment Loop (Seller -> Rider)
1. **MPESA Logic**: VibeSoko monitors incoming SMS (via SMS Forwarder) or manual entry.
